...
- [x] Set and Forget table - hypothetical points if manager made no transfers all season:
  - [x] Calculate points from GW1 squad only (no transfers)
  - [x] Use original captain choice; fall back to vice-captain if captain unavailable
  - [x] Apply TC/BB chips; ignore Wildcard/Free Hit (use GW1 squad for those weeks)
  - [x] Show comparison: actual points vs set-and-forget points
  - [x] Highlight managers who would have done better/worse without transfers
